...
<div class="flex items-center justify-center h-screen">
    <div class="w-10/12 md:w-full max-w-sm p-4 bg-secondary rounded-lg shadow sm:p-6 md:p-8">
        <form action="{{ route('generate') }}" method="POST" class="space-y-6">
            ...
            @csrf
            <div>
                <label for="phone_number" class="block mb-2 text-sm font-medium text-purple">Masukkan Nomor Telepon</label>
                <input id="phone_number" type="text" class="bg-transparent border-2 border-purple text-purple text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 " name="phone_number" required autocomplete="phone_number" autofocus placeholder="Masukkan Nomor telepon yang terdaftar">
                @error('phone_number')

...
<div class="p-4 rounded-lg mt-14 bg-gray-100 border border-purple dark:border-none shadow-md dark:bg-secondary">
    <div class="flex justify-between">
        <div class="text-primary dark:text-purple m-4 font-semibold text-2xl tracking-wide w-1/2">Data User</div>
        <div class="mt-10 mb-4 w-1/2">
            ...
            <div class="flex justify-between mt-4">
                <div class="text-primary dark:text-purple mb-4 flex items-end">Hasil pencarian dari: {{ $searchQuery }}</div>
                <a href="{{route('admin.dataUser')}}" class="text-blue-600">Clear search</a>
            </div>
            @endif
        </div>
    </div>
                                        ...
                                        <form action="{{ route('admin.updateUser', $users->id) }}" method="POST">
                                            ...
                                            <div class="grid gap-4 mb-4 sm:grid-cols-2">
                                                <div>
                                                    ...
                                                    <input type="text" name="id" id="id" value="{{$users->id}}" class="bg-gray-100 dark:bg-primary text-primary dark:text-purple text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" disabled>
                                                </div>
                                                <div>
                                                    <label for="username" class="block mb-2 text-sm font-medium text-primary dark:text-purple">Nama Dealer</label>
                                                    <input type="text" name="username" id="username" value="{{$users->username}}" class="bg-gray-100 dark:bg-primary text-primary dark:text-purple text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" disabled>
                                                </div>
                                                <div>
                                                    <label for="email" class="block mb-2 text-sm font-medium text-primary dark:text-purple">Alamat Dealer</label>
                                                    <input type="text" name="email" id="email" value="{{$users->email}}" class="bg-gray-100 dark:bg-primary text-primary dark:text-purple text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" disabled>
                                                </div>
                                                <div>
                                                    <label for="role" class="block mb-2 text-sm font-medium text-primary dark:text-purple">Role</label>
                                                    <select id="role" name="role" class="bg-gray-100 dark:bg-primary text-primary dark:text-purple text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:border-gray-600 dark:placeholder-gray-400 dark:focus:ring-primary-500 dark:focus:border-primary-500">
